...
content.weld = function(dom, pages) {
  ...
  Object.keys(pages).every( function (i) {

    // TODO: Do a pre-check for mtime-based caching here.
    //
    // The obvious approach is to read the maximum mtime for existing resources
    // as compared to the files they're supposed to generate. However, this
    // is complicated by the fact that if a *new* resource is created then the
    // ToC changes meaning, yes, all the pages must be generated again.
    //
    // So probably what we'll want to do is:
    // * Generated an expected list of existing files.
    // * If all those files exist, break out the mtime caching algo.
    // * If any of the resources are new, generate ALL the things.

    var page = pages[i],
        mtime = page.mtime,
        newLink = page.metadata.newLink;

    if (path.existsSync(newLink)) {
      // check the mtimes
      var newTime = fs.statSync(newLink).mtime;

      // Do we need to render?
      page.generate = Boolean(mtime > newTime);
    }
    else {
      cached = false;
      // short-circuit out, since we have to generate all the pages anyway.
      return false;
    }
  });
  ...
  Object.keys(pages).forEach( function (i) {
    ...
    // TODO: Set up conditional rendering based on `cached` and pages[i].render.
    if (cached) {
      if (!pages[i].generate) {
        delete pages[i];
        return;
