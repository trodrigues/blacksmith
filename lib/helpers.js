...
// }
//
// TODO: Just put this in the loader. This is too specific to be a "helper."
helpers.dirToContent = function (src, files, resolve) {
  ...
  Object.keys(files).forEach(function (f) {
    ...
    if (path.basename(f) === "page.json") {
      ...
      // TODO: Do a stat and attach content mtime data somewhere in here
      // for the page.json. This should overwrite existing mtime data only
      // if it's a more recent data than that existing.

      content[p].mtime = (function () {
        var mtime = fs.statSync(p).mtime;

        if (!content[p].mtime
        || (content[p].mtime && content[p].mtime < mtime) ) {
          content[p].mtime = mtime;
        }
      })();

      // TODO: Also calculate the "new path" here for later mtime comparisons
      content[p].metadata.link = p;
      content[p].metadata.newLink = path.normalize(p.replace(path.resolve(smith.src), path.resolve(smith.dst)) + '/index.html');
      ...
      // TODO: Do a stat and attach content mtime data somewhere in here
      // for the content.md. This should overwrite existing mtime data only
      // if it's a more recent data than that existing.
